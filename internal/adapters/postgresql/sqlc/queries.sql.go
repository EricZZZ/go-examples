...
const updateProductQuantityByID = `-- name: UpdateProductQuantityByID :one
UPDATE products SET quantity = $1 WHERE id = $2 RETURNING id, name, price_in_centers, quantity, created_at
`

type UpdateProductQuantityByIDParams struct {
	Quantity int32 `json:"quantity"`
	ID       int64 `json:"id"`
}

func (q *Queries) UpdateProductQuantityByID(ctx context.Context, arg UpdateProductQuantityByIDParams) (Product, error) {
	row := q.db.QueryRow(ctx, updateProductQuantityByID, arg.Quantity, arg.ID)
	var i Product
	err := row.Scan(
		&i.ID,
		&i.Name,
